...
export const metadata = {
  title: "Wydarzenia · Serwis Informacyjny Konopiska",
  description: "Zapowiedzi wydarzeń, festiwale, koncerty plenerowe i inicjatywy kulturalne w Gminie Konopiska.",
}
...
export default function WydarzeniaPage() {
  ...
  // Filter items matching Wydarzenia & Kultura
  const filteredItems = newsItems.filter(
    (item) => item.category === "Wydarzenia" || item.category === "Kultura"
  )

  const lead = filteredItems[0]
  const rest = filteredItems.slice(1)

  // Recommendation items from other categories to make page look premium and complete
  const recommendations = newsItems
    .filter((item) => item.category !== "Wydarzenia" && item.category !== "Kultura")
    .slice(0, 3)
  ...
  return (
    <main className="min-h-screen bg-background">
      ...
      <PageHero
        eyebrow="Kalendarz gminny"
        title="Wydarzenia i Kultura"
        description="Festiwale plenerowe, koncerty, imprezy sportowo-rekreacyjne, obchody rocznicowe oraz bogata oferta kulturalna Gminnego Centrum Kultury i Sportu."
        breadcrumb={[
          { label: "Serwis", href: "/" },
          { label: "Wydarzenia" },
        ]}
      ...
      <section className="mx-auto max-w-[90rem] px-4 py-16 sm:px-6">
        ...
        {lead ? (
          ...
          >
            ...
            <div className="flex flex-col justify-center gap-5 p-8 md:p-12">
              ...
              <div className="mt-2 flex items-center gap-2 text-xs text-muted-foreground">
                ...
                <span>{lead.readingTime ? `${lead.readingTime} min` : "3 min"} czytania</span>
              </div>
          ...
          <div className="rounded-3xl border border-border bg-card p-12 text-center">
            <p className="text-muted-foreground">Brak wydarzeń w tym momencie.</p>
          </div>
        )}
            ...
            <div className="mb-8 flex items-end justify-between gap-6 border-b border-border pb-4">
              <div>
                ...
                <h3 className="mt-2 font-serif text-2xl font-semibold tracking-tight text-foreground">
                  Zapowiedzi i relacje kulturalne
                </h3>
              </div>
            </div>
                ...
                <li key={item.slug}>
                  ...
                  >
                    ...
                    <div className="hidden text-xs text-muted-foreground md:block">
                      ...
                      <p className="mt-1">{item.readingTime ? `${item.readingTime} min` : "3 min"}</p>
                    </div>
              ...
              {recommendations.map((item) => (
                ...
                >
                  ...
                  <div className="flex flex-1 flex-col p-6">
                    ...
                    <h4 className="mt-3 font-serif text-lg font-semibold leading-snug text-foreground group-hover:text-primary line-clamp-2">
                      {item.title}
                    </h4>
                    ...
                    <div className="mt-6 flex items-center justify-between text-xs text-muted-foreground border-t border-border/50 pt-4">
                      ...
                      <span>{item.readingTime ? `${item.readingTime} min` : "3 min"} czytania</span>
                    </div>
                  </div>
                </Link>
              ))}
